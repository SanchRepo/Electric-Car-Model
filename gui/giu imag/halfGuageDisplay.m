...
if (score < 0)
    disp('Has to be greater than 0rpm!');
else if (score > 400)
        disp('Cannot read more than 400rpm');
    else
        ...
        text(x(1),y(1),'400rpm','FontSize',12)
        text(x(length(angle)),y(length(angle)),'0rpm','FontSize',12)
        text(x(round(length(angle)/2)),y(round(length(angle)/2)),'200','FontSize',12)
        axis equal
        axis off
        hold on
        ...
        scoreString = strcat(num2str(score),'rpm');
